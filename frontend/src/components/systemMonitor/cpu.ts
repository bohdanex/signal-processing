...
export const cpuChartOptions = createChartOptions(0, 100);

export function getCPU_chartData(workloadData: Array<Workload & { time: number }>) {
  return {
    ...
    datasets: [
      {
        label: "CPU Usage %",
        data: workloadData.map((d) => d.CPU?.UsagePercent ?? 0),
